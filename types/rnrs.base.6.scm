...
If q is a rational number objects, then (integer? q) is true if and only if (= (denominator q) 1) is true. If q is not a rational number object, then (integer? q) is #f."))
 ...
 ((group
    ...
    ((name . "atan")
     ...
     (tags pure)))
  (desc . "These procedures compute the usual transcendental functions. The exp procedure computes the base-e exponential of z . The log procedure with a single argument computes the natural logarithm of z (not the base-ten logarithm); (log z1 z2) computes the base-z2 logarithm of z1. The asin, acos, and atan procedures compute arcsine, arccosine, and arctangent, respectively. The two-argument variant of atan computes (angle (make-rectangular x2 x1)). These procedures may return inexact results even when given exact arguments."))
 ((group
    ((name . "numerator") (signature lambda ((rational? q)) integer?) (tags pure))
    ((name . "denominator")
     (signature lambda ((rational? q)) integer?)
     (tags pure)))
  (desc . "These procedures return the numerator or denominator of their argument; the result is computed as if the argument was represented as a fraction in lowest terms. The denominator is always positive. The denominator of 0 is defined to be 1."))
 ((group
    ((name . "make-polar")
    ...
    ((name . "angle")
     ...
     (tags pure)))
  (desc . "Suppose a1, a2, a3, and a4 are real numbers, and c is a complex number such that the following holds:
c = a1 + a2*i = a3*e^(i*a4)
