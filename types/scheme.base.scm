(((name . "*") (signature lambda ((number? z) ...) number?) (tags pure) (desc . "+, * These procedures return the sum or product of their arguments."))
 ((name . "+") (signature lambda ((number? z) ...) number?) (tags pure) (desc . "+, * These procedures return the sum or product of their arguments."))
 ((name . "-") (signature lambda ((number? z) ...) number?) (tags pure) (desc . "-, / With two or more arguments, these procedures return the difference or quotient of their arguments, associating to the left. With one argument, however, they return the additive or multiplicative inverse of their argument. It is an error if any argument of / other than the first is an exact zero. If the first argument is an exact zero, an implementation may return an exact zero unless one of the other arguments is a NaN."))
 ((name . "/")
  (signature lambda ((number? z1) (number? z2) ...) number?)
  (tags pure)
  (desc . "-, / With two or more arguments, these procedures return the difference or quotient of their arguments, associating to the left. With one argument, however, they return the additive or multiplicative inverse of their argument. It is an error if any argument of / other than the first is an exact zero. If the first argument is an exact zero, an implementation may return an exact zero unless one of the other arguments is a NaN."))
 ((name . "<")
  (signature lambda ((real? x1) (real? x2) (real? x3) ...) boolean?)
  (tags pure)
  (desc . "=, <, >, <=, >= These procedures return #t if their arguments are (respectively): equal, monotonically increasing, monotonically decreasing, monotonically non-decreasing, or monotonically non-increasing, and #f otherwise. If any of the arguments are +nan.0, all the predicates return #f. They do not distinguish between inexact zero and inexact negative zero. These predicates are transitive. Note: While it is not an error to compare inexact numbers using these predicates, the results are unreliable because a small inaccuracy can affect the result; this is especially true of = and zero?. When in doubt, consult a numerical analyst."))
 ((name . "<=")
  (signature lambda ((real? x1) (real? x2) (real? x3) ...) boolean?)
  (tags pure)
  (desc . "=, <, >, <=, >= These procedures return #t if their arguments are (respectively): equal, monotonically increasing, monotonically decreasing, monotonically non-decreasing, or monotonically non-increasing, and #f otherwise. If any of the arguments are +nan.0, all the predicates return #f. They do not distinguish between inexact zero and inexact negative zero. These predicates are transitive. Note: While it is not an error to compare inexact numbers using these predicates, the results are unreliable because a small inaccuracy can affect the result; this is especially true of = and zero?. When in doubt, consult a numerical analyst."))
 ...
 ((name . "assoc")
  ...
  (subsigs (= (lambda (a b) *)))
  (tags pure)
  (desc . "assq, assv, assoc It is an error if alist (for \"association list\") is not a list of pairs. These procedures find the first pair in alist whose car field is obj , and returns that pair. If no pair in alist has obj as its car, then #f (not the empty list) is returned. The assq procedure uses eq? to compare obj with the car fields of the pairs in alist , while assv uses eqv? and assoc uses compare if given and equal? otherwise. Rationale: Although they are often used as predicates, memq, memv, member, assq, assv, and assoc do not have question marks in their names because they return potentially useful values rather than just #t or #f."))
 ((name . "assq")
 ...
 ((name . "bytevector?")
  (signature lambda (obj) boolean?)
  (tags pure predicate))
 ((name . "caar") (signature lambda ((pair? pair)) *) (tags pure) (desc . "caar, cadr, cdar, cddr These procedures are compositions of car and cdr"))
 ((name . "cadr") (signature lambda ((pair? pair)) *) (tags pure) (desc . "caar, cadr, cdar, cddr These procedures are compositions of car and cdr"))
 ...
 ((name . "char<=?")
  ...
  (tags pure)
  (desc . "char=?, char<?, char>?, char<=?, char>=? These procedures return #t if the results of passing their arguments to char->integer are respectively equal, monotonically increasing, monotonically decreasing, monotonically non-decreasing, or monotonically non-increasing. These predicates are required to be transitive."))
 ((name . "char<?")
  (signature lambda ((char? char1) (char? char2) (char? char3) ...) boolean?)
  (tags pure)
  (desc . "char=?, char<?, char>?, char<=?, char>=? These procedures return #t if the results of passing their arguments to char->integer are respectively equal, monotonically increasing, monotonically decreasing, monotonically non-decreasing, or monotonically non-increasing. These predicates are required to be transitive."))
 ...
 ((name . "close-input-port")
  ...
  (desc . "close-port, close-input-port, close-output-port Closes the resource associated with port, rendering the port incapable of delivering or accepting data. It is an error to apply the last two procedures to a port which is not an input or output port, respectively. Scheme implementations may provide ports which are simultaneously input and output ports, such as sockets; the close-input-port and close-output-port procedures can then be used to close the input and output sides of the port independently. These routines have no effect if the port has already been closed."))
 ...
 ((name . "cons") (signature lambda (obj1 obj2) pair?) (tags pure) (desc . "Returns a newly allocated pair whose car is obj1 and whose cdr is obj2. The pair is guaranteed to be different(sense of eqv?) from every existing object."))
 ...
 ((name . "denominator")
  ...
  (tags pure)
  (desc . "numerator, denominator These procedures return the numerator or denominator of their argument; the result is computed as if the argument was represented as a fraction in lowest terms. The denominator is always positive. The denominator of 0 is defined to be 1."))
 ((name . "do")
  (signature
   syntax-rules
   ()
 ...
 ((name . "exact?")
  ...
  (tags pure)
  (supertypes number?)
  (desc . "exact?, inexact? These numerical predicates provide tests for the exactness of a quantity. For any Scheme number, precisely one of these predicates is true."))
 ((name . "expt")
  (signature lambda ((number? z1) (number? z2)) number?)
  (tags pure)
 ...
 ((name . "gcd")
  ...
  (tags pure)
  (desc . "gcd, lcm These procedures return the greatest common divisor or least common multiple of their arguments. The result is always non-negative."))
 ...
 ((name . "include") (signature syntax-rules () ((_ string1 string2 ...))) (desc . "include, include-ci Both include and include-ci take one or more filenames expressed as string literals, apply an implementation-specific algorithm to find corresponding files, read the contents of the files in the specified order as if by repeated applications of read, and effectively replace the include or include-ci expression with a begin expression containing what was read from the files. The difference between the two is that include-ci reads each file as if it began with the #!fold-case directive, while include does not. Note: Implementations are encouraged to search for files in the directory which contains the including file, and to provide a way for users to specify other directories to search."))
 ((name . "include-ci") (signature syntax-rules () ((_ string1 string2 ...))) (desc . "include, include-ci Both include and include-ci take one or more filenames expressed as string literals, apply an implementation-specific algorithm to find corresponding files, read the contents of the files in the specified order as if by repeated applications of read, and effectively replace the include or include-ci expression with a begin expression containing what was read from the files. The difference between the two is that include-ci reads each file as if it began with the #!fold-case directive, while include does not. Note: Implementations are encouraged to search for files in the directory which contains the including file, and to provide a way for users to specify other directories to search."))
 ...
 ((name . "inexact?")
  ...
  (supertypes number?)
  (desc . "exact?, inexact? These numerical predicates provide tests for the exactness of a quantity. For any Scheme number, precisely one of these predicates is true."))
 ((name . "input-port-open?") (signature lambda ((input-port? port)) boolean?) (desc . "input-port-open?, output-port-open? Returns #t if port is still open and capable of performing input or output, respectively, and #f otherwise."))
 ((name . "input-port?")
  (signature lambda (obj) boolean?)
  (tags pure predicate)
  (supertypes port?))
 ...
 ((name . "lcm") (signature lambda ((integer? n) ...) integer?) (tags pure) (desc . "gcd, lcm These procedures return the greatest common divisor or least common multiple of their arguments. The result is always non-negative."))
 ...
 ((name . "list->string")
  ...
  (subsigs
    (list (list (char? c))))
  (desc . "string->list, list->string The string->list procedure returns a newly allocated list of the characters of string between start and end. list->string returns a newly allocated string formed from the elements in the list list. In both procedures, order is preserved. string->list and list->string are inverses so far as equal? is concerned."))
 ((name . "list->vector")
  (signature lambda ((list? list)) vector?)
  (tags pure)
  (desc . "vector->list, list->vector The vector->list procedure returns a newly allocated list of the objects contained in the elements of vector between start and end. The list->vector procedure returns a newly created vector initialized to the elements of the list list. In both procedures, order is preserved."))
 ...
 ((name . "modulo")
  (signature lambda ((integer? n1) (integer? n2)) integer?)
  (tags pure)
  (desc . "quotient, remainder, modulo The quotient and remainder procedures are equivalent to truncate-quotient and truncate-remainder, respectively, and modulo is equivalent to floor-remainder."))
 ...
 ((name . "remainder")
  ...
  (tags pure)
  (desc . "quotient, remainder, modulo The quotient and remainder procedures are equivalent to truncate-quotient and truncate-remainder, respectively, and modulo is equivalent to floor-remainder."))
 ((name . "reverse") (signature lambda ((list? list)) list?) (tags pure) (desc . "Returns a newly allocated list consisting of the elements of list in reverse order."))
 ((name . "round") (signature lambda ((real? x)) integer?) (tags pure) (desc . "floor, ceiling, truncate, round These procedures return integers. The floor procedure returns the largest integer not larger than x. The ceiling procedure returns the smallest integer not smaller than x, truncate returns the integer closest to x whose absolute value is not larger than the absolute value of x, and round returns the closest integer to x, rounding to even when x is halfway between two integers. Rationale: The round procedure rounds to even for consistency with the default rounding mode specified by the IEEE 754 IEEE floating-point standard. Note: If the argument to one of these procedures is inexact, then the result will also be inexact. If an exact value is needed, the result can be passed to the exact procedure. If the argument is infinite or a NaN, then it is returned."))
 ((name . "set!")
 ...
 ((name . "set-car!") (signature lambda ((pair? pair) obj) undefined) (desc . "Stores obj in the car field of pair."))
 ((name . "set-cdr!") (signature lambda ((pair? pair) obj) undefined) (desc . "Stores obj in the cdr field of pair."))
 ((name . "square") (signature lambda ((number? z)) number?) (tags pure) (desc . "Returns the square of z. This is equivalent to (* z z)."))
 ((name . "string") (signature lambda ((char? char) ...) string?) (tags pure) (desc . "Returns a newly allocated string composed of the arguments. It is analogous to list."))
 ((name . "string->list")
  (signature
   case-lambda
   (((string? string)) list?)
 ...
 ((name . "string->vector")
  ...
  (desc . "vector->string, string->vector It is an error if any element of vector between start and end is not a character. The vector->string procedure returns a newly allocated string of the objects contained in the elements of vector between start and end. The string->vector procedure returns a newly created vector initialized to the elements of the string string between start and end. In both procedures, order is preserved."))
 ((name . "string-append")
 ...
 ((name . "textual-port?")
  ...
  (tags pure predicate)
  (supertypes port?))
 ((name . "truncate") (signature lambda ((real? x)) integer?) (tags pure) (desc . "floor, ceiling, truncate, round These procedures return integers. The floor procedure returns the largest integer not larger than x. The ceiling procedure returns the smallest integer not smaller than x, truncate returns the integer closest to x whose absolute value is not larger than the absolute value of x, and round returns the closest integer to x, rounding to even when x is halfway between two integers. Rationale: The round procedure rounds to even for consistency with the default rounding mode specified by the IEEE 754 IEEE floating-point standard. Note: If the argument to one of these procedures is inexact, then the result will also be inexact. If an exact value is needed, the result can be passed to the exact procedure. If the argument is infinite or a NaN, then it is returned."))
