(((name . "*")
 ...
 ((name . "bytevector?")
  ...
  (tags pure predicate)
  (desc . "Returns #t if obj is a bytevector. Otherwise, #f is returned."))
...
Copies the characters of string from between start and end to string to, starting at at. The order in which characters are copied is unspecified, except that if the source and destination overlap, copying takes place as if the source is first copied into a temporary string and then into the destination. This can be achieved without allocating storage by making sure to copy in the correct direction in such circumstances."))
 ...
 ((group
    ...
    ((name . "current-output-port")
     (signature lambda () output-port?)
     (tags parameter)))
  (desc . "Returns the current default input port, output port, or error port (an output port), respectively. These procedures are parameter objects, which can be overridden with parameterize (see section 4.2.6). The initial bindings for these are implementation-defined textual ports."))
 ((group
 ...
 ((group
    ...
    ((name . "port?")
     (signature lambda (obj) boolean?)
     (tags pure predicate)))
  (desc . "These procedures return #t if obj is an input port, output port, textual port, binary port, or any kind of port, respectively. Otherwise they return #f."))
