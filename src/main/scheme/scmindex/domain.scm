  ...
  (begin
    ...
    (define (jsonify-signature sig)
      (define (jsonify-lambda l)
        `((type . "function")
          (variants . #(((params . ,(list->vector (map jsonify-lambda-param (car l))))
                         (return . ,(jsonify-lambda-return (cadr l))))))))
      (define (jsonify-case-lambda l)
        `((type . "function")
          (variants . #(,@(map
                            (lambda (e)
                              `((params . ,(list->vector (map jsonify-lambda-param (car e))))
                                (return . ,(jsonify-lambda-return (cadr e)))))
                            l)))))
      ...
      (define jsonifier
        (case (car sig)
          ...
          ((case-lambda) jsonify-case-lambda)
