...
(define (%read-error? x) ::boolean
  (read-error? x))
(define (valid-number? s) ::boolean
...
(define (json-whitespace? char) ::boolean
  (case char
    ((#\x20 ; Space
      #\x09 ; Horizontal tab
...
(define (port->generator port ::input-port) ::procedure
  (let ((count 0)
...
(define (gcons head generator ::procedure) ::procedure
...
(define (%json-tokens generator ::procedure)

  (define (maybe-ignore-whitespace generator ::procedure) 
    (let loop ((char  (generator)))
      (if (json-whitespace? char)
          (loop (generator))
          char)))

  (define (expect-null generator ::procedure) ::void
    (expect (generator) #\u)
    (expect (generator) #\l)
    (expect (generator) #\l))

  (define (expect-true generator ::procedure) ::void
    (expect (generator) #\r)
    (expect (generator) #\u)
    (expect (generator) #\e))

  (define (expect-false generator ::procedure) ::void
    (expect (generator) #\a)
    (expect (generator) #\l)
    (expect (generator) #\s)
    (expect (generator) #\e))

  (define (maybe-char generator ::procedure) 
    (let ((char  (generator)))
  ...
  (define (read-unicode-escape generator ::procedure) ::number
  ...
  (define (read-json-string generator ::procedure)
    (let loop ((char (generator))
               (out '()))
  ...
  (define (maybe-read-number generator ::procedure)
  ...
  (let ((char (generator)))
    (if (eof-object? char)
        ...
        ;; NOTE default impl returned just eof-object
        (lambda () char)
        (begin
...
(define (%json-generator tokens ::procedure) ::procedure
  ...
  (define (array-maybe-continue tokens ::procedure k) ::procedure
    (lambda ()
      (let ((token (tokens)))
        (case token
  ...
  (define (array-start tokens ::procedure k) ::procedure
    (lambda ()
      (handle-limit!)
      (let ((token (tokens)))
  ...
  (define (object-maybe-continue tokens ::procedure k) ::procedure
    (lambda ()
      (let ((token (tokens)))
        (case token
  ...
  (define (object-colon tokens ::procedure k) ::procedure
    (lambda ()
      (let ((token (tokens)))
        (if (eq? token 'colon)
            (let ((token (tokens)))
  ...
  (define (object-start tokens ::procedure k) ::procedure
    (lambda ()
      (handle-limit!)
      (let ((token (tokens)))
        (cond
  ...
  (define (start tokens ::procedure k)
    (let ((token (tokens)))
      (if (eof-object? token)
          (values token k)
          (cond
  ...
  (define (make-trampoline-generator tokens ::procedure) ::procedure
...
(define (%json-fold proc 
                    ...
                    array-end ::procedure
                    object-start ::procedure 
                    object-end ::procedure 
                    seed 
                    port-or-generator)
...
(define (json-accumulator accumulator ::procedure) ::procedure

  (define (write-json-char char accumulator ::procedure)
    (case char
      ((#\x00) (accumulator "\\u0000"))
      ((#\") (accumulator "\\\""))
      ((#\\) (accumulator "\\\\"))
  ...
  (define (write-json-string string accumulator ::procedure)
    (accumulator #\")
    (string-for-each
  ...
  (define (write-json-value obj accumulator ::procedure)
    (cond
  ...
  (define (object-start k) ::procedure
  ...
  (define (object-value k) ::procedure
    (lambda (accumulator event)
      (accumulator #\:)
      (case (car event)
        ((json-value)
  ...
  (define (object-maybe-continue k) ::procedure
    (lambda (accumulator event)
      (case (car event)
        ((json-value)
         (accumulator #\,)
  ...
  (define (array-start k) ::procedure
    (lambda (accumulator event)
      (accumulator #\[)
      (case (car event)
        ((json-value)
  ...
  (define (array-maybe-continue k) ::procedure
    (lambda (accumulator event)
      (case (car event)
        ((json-value)
         (accumulator #\,)
  ...
  (define (start accumulator ::procedure event ::pair)
    (case (car event)
      ((json-value)
       (write-json-value (cdr event) accumulator)
       raise-invalid-event)
...
(define (%json-write obj accumulator ::procedure)
  ...
  (define (write obj accumulator ::procedure)
    (cond
     ((or (eq? obj 'null)
          (boolean? obj)
          (string? obj)
...
(define (port->accumulator port ::output-port)
