...
* https://scheme-index.aartaka.me

Rendered HTML version of this readme is available at https://schemeorg-community.github.io/index.scheme.org/.
...
| group
| A list of identifiers under the group. Each entry must have format as described under a table for single idenfier entry, except that it needs no `desc`-ription for the virtue of having a group one.
...
Currently used tags are: `pure` for procedures modifying no state; and `predicate` for boolean-returning single-argument procedures checking conformance of a value with certain criteria.
...
* Running `validatedata-native.sh` (requires chicken scheme installation);

* Running `validatedata-docker.sh` (requires docker installation).
